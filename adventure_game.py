...
def monsters():
    list = ['Goblin the Illtempered Creep', 'Old Man Grabhands',
            'The Leprechaun of infinite Terror']
    global random_item
    if not random_item:
...
def weapons():
    fightstuff = ['The Sword of Ogoroth', 'The Wand of Mischeif',
                  'The Hankercheif of Darkness']
...
def intro():
    ...
    print_pause("Rumor has it that " + (monsters()) + " is somewhere "
...
def knock():
    ...
    if 'fightstuff' in items:
        choice = valid_input("Would you like to fight(y) or flee(n)?\n"
                             "Please type 'y' or 'n'\n", "y", "n")
        if choice == "y":
            print_pause("As " + (monsters()) + " moves to attack, "
